...
'''
'''
Every great chef knows that lucky numbers are positive integers whose decimal representations contain only the 
lucky digits 4 and 7. For example, numbers 47, 744, 4 are lucky and 5, 17, 467 are not.

Our chef has recently returned from the Lucky country. He observed that every restaurant in the Lucky country 
had a lucky number as its name.
He believes that having a lucky number as a restaurant name can indeed turn out to be very lucky.

Our chef believes that it is possible to make a lucky number having N digits even luckier. Any number following 
the rules below is called Lucky lucky number -
...
First line contains T, number of test cases. Each of the next T lines contains a number N, the number of 
digits in the Lucky lucky number to be formed.
...
def get_num_of_4(num_digits):
   
   #Maximize the equation 7a + 4b = num_digits - Maximize a, minimize b
   b = 0
   rem = 1
   while(rem > 0):
       rem = num_digits - 4*b
