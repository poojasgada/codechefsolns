...
before they left so only a subset of the n jobs remain. The Chef and his assistant
 divide up the remaining jobs in the following manner. The Chef takes the 
 unfinished job with least index, the assistant takes the unfinished job with the 
 second least index, the Chef takes the unfinished job with the third least index,
  etc. That is, if the unfinished jobs were listed in increasing order of their 
  index then the Chef would take every other one starting with the first job in 
  the list and the assistant would take every other one starting with the second job on in the list.

The cooks logged which jobs they finished before they left. Unfortunately, these 
jobs were not recorded in any particular order. Given an unsorted list of finished 
jobs, you are to determine which jobs the Chef must complete and which jobs his 
...
two numbers n,m satisfying . Here, n is the total number of jobs 
...
#May have to check if the new lines are correct
def assign_jobs(num_jobs, num_completed_jobs, completed_jobs_list):
    chef_jobs = []
    assistant_jobs = []
    completed_jobs_list.sort()
    chef = 1
    
    for i in range(1, num_jobs+1):
        try:
            completed_jobs_list.index(i)
        except:
            if(chef == 1):
                chef_jobs.append(i)
                chef = 0
...
def get_input():
    ...
    for num in range(1, num_test_cases+1):
        num_jobs, num_completed_jobs = map(int, raw_input().split())
        completed_jobs_list = map(int, raw_input().split())
        assign_jobs(num_jobs, num_completed_jobs, completed_jobs_list)
