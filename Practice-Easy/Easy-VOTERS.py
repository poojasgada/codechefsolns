...
def find_voters():
    num_count_list = map(int, sys.stdin.readline().split())
    
    #Add all the votes into a dictionary, to efficiently keep track of the count of votes
    voter_dict = {}
    for i in range(0, 3):
        for j in range(0, num_count_list[i]):
            num = int(sys.stdin.readline().strip())
            if num in voter_dict.keys():
                voter_dict[num] += 1
            else:
                voter_dict[num] = 1
    
    #Once we have the count of the votes, and found the >=2 ones, add them into a list and sort them
    voter_list = []
    for k in voter_dict.keys():
        if voter_dict[k] >=2:
            voter_list.append(k)
