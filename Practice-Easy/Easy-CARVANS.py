...
#May give time exceeded error, if it does, then have to use a different way to input car maximum speeds

def cars_on_max_speed(car_speed_list, num_cars):
    car_count = 1
    temp = 0
    for i in range(1, num_cars):
        temp = min(car_speed_list[i], car_speed_list[i-1])
        
        if(temp == car_speed_list[i]):
            car_count += 1
        else:
            car_speed_list[i]  = temp
...
def get_input():
    ...
    for i in range(1, num_test_cases+1):
        num_cars = int(input())
        car_speed_list = map(int, raw_input().split())
        cars_on_max_speed(car_speed_list, num_cars)
